...
export async function handleGatewayAdmin(request: Request, env: Env): Promise<Response> {
  ...
  try {
    ...
    return Response.json({ ok: true, identities: config.identities.length });
...
<div class="grid"><div><label>身份路径<input id="ruleSlug" placeholder="dalaogong"></label><label>匹配模型名<input id="ruleMatch" placeholder="*haiku*"></label><label>改写为（可选，留空即透传）<input id="ruleModel" placeholder="compat/moonshot/kimi-k2"></label></div><div><label>召回<select id="ruleMemory"><option value="">跟随身份</option><option value="off">不召回</option><option value="request">召回</option></select></label><label>记录<select id="ruleRecord"><option value="">跟随身份</option><option value="false">不记录</option><option value="true">记录</option></select></label><button id="addRule">添加到草稿</button></div></div></section>
<section><h2>4 · 检查并保存</h2><textarea id="config" rows="22" spellcheck="false" aria-label="网关配置 JSON">{"version":2,"providers":{},"identities":[]}</textarea><button id="save">保存配置</button><div id="status" role="status" aria-live="polite"></div>
...
el('example').onclick=()=>{show({version:2,providers:{anthropic:{gateway:'default',provider:'anthropic',paths:{messages:'v1/messages'}},openai:{gateway:'default',provider:'openai'},compat:{gateway:'default',provider:'compat'}},identities:[{slug:'dalaogong',namespace:'companion-a',keys:['CHATBOX_API_KEY'],provider:'anthropic',memory:'request',record:true,anthropicThinking:'passthrough',models:[{match:'*haiku*',memory:'off',record:false},{match:'anthropic/claude-opus-4-5'}]},{slug:'erlaogong',namespace:'companion-b',keys:['CHATBOX_API_KEY'],provider:'openai',memory:'request',record:true,anthropicThinking:'passthrough',models:[{match:'*mini*',memory:'off',record:false}]}]});status('已填入示例草稿。线路名要和 CF AI Gateway 里的 provider 对上，模型 ID 换成你的。')};
</script></body></html>`;
