...
import { allowedProfiles, loadConfig, type GatewayProfile, type Protocol } from "./config";
import { appendMemory, classifyTurn, inputItems, validateBody, type Body } from "./protocol";
...
export async function recallPatch(env: Env, profile: GatewayProfile, query: string, ctx: ExecutionContext): Promise<string> {
  const precious = await listPrecious(env.DB, { namespace: profile.namespace, limit: 20 });
  const recall = await runRecall(env, { namespace: profile.namespace, query,
  ...
  const budget = profile.maxMemoryChars || 6000;
...
export async function handleGateway(request: Request, env: Env, ctx: ExecutionContext, protocol: Protocol): Promise<Response> {
  ...
  const profile = allowedProfiles(config, auth).find(p => p.alias === body.model);
  if (!profile) return gatewayError(protocol, "Unknown or unauthorized model alias. Configure /admin/gateway and check /v1/models.", 403);
  if (!profile.routes[protocol]) return gatewayError(protocol, `This alias has no ${protocol} route`, 400);
  if (protocol === "responses" && profile.memory === "request" && (body.previous_response_id || body.conversation || inputItems(body, protocol).some(x => x.type === "item_reference"))) {
    return gatewayError(protocol, "Request-only memory requires stateless Responses input: send full history without previous_response_id, conversation or item_reference; or use a memory-off alias.", 400);
  }
  const turn = classifyTurn(body, protocol, request.headers.get("x-aelios-purpose") === "auxiliary");
  let patch = "";
  let memoryStatus = profile.memory === "off" ? "off" : turn.kind !== "human" ? "skipped" : "empty";
  const thinkingCompatible = protocol !== "messages" || profile.anthropicThinking === "drop_block" || body.thinking?.type === "disabled";
  if (profile.memory === "request" && turn.kind === "human" && turn.text && thinkingCompatible) {
    try { patch = await recallPatch(env, profile, turn.text, ctx); memoryStatus = patch ? "injected" : "empty"; }
    catch { memoryStatus = "unavailable"; console.error("gateway recall unavailable", { profile: profile.alias }); }
  } else if (!thinkingCompatible && profile.memory === "request") memoryStatus = "thinking-passthrough";
  const payload = appendMemory(body, protocol, patch);
  if (protocol === "responses" && profile.memory === "request") payload.store = false;
  const exchange = profile.record ? await prepareExchange(request, body, profile, protocol, turn, auth.profile.source) : null;
  try {
    const { response: upstream, target } = await callGatewayUpstream(env, config, profile, protocol, request, payload);
    const headers = new Headers(upstream.headers);
    headers.set("x-aelios-profile", profile.alias);
